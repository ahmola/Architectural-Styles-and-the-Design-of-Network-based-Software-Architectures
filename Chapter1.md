...
REST라는 아키텍처 스타일을 정의하기에 앞서, 소프트웨어 아키텍처란 무엇인지, 그리고 스타일과 패턴에 대해서 설명합니다.

	시스템은 컴포넌트, 커넥터, 데이터로 구성된다.
	관계(Configuration)은 제약(Constraints)에 의해 규정되며,
	이렇게 규정된 제약의 조합이 품질로 이어지고,
	제약의 패턴화가 아키텍처의 스타일이 된다.

1.1 : 아키텍처는 런타임(실행 시)의 추상화, 코드의 구조가 아닌 **컴포넌트들이 실행 중에 어떻게 서로 상호작용하는가**라는 동작을 다루는 것이 본질
1.2 : 아키텍처의 세 가지 요소는 컴포넌트, 커넥터, 데이터로 이루어져있다. 대부분은 컴포넌트만을 신경쓰며, 단순 선과 상자로만 표현하려고 하나 이것은 잘못되었다는 것을 강조
1.3 : 아키텍처의 세 요소가 런타임에서의 관계인 실제 어떻게 돌아가는지가 Configuration. 이 때, 실행 중인 active와 가능한 전체 구성인 potential을 구분해야함을 강조
1.4 : 시스템의 기능적 속성과 비기능적 속성이 아키텍처에 있음을 언급하며, 제약으로 인해 품질이 생기는 역설적인 상황을 설명
1.5 : 이렇게 만들어진 제약의 모음(또는 집합)을 일컬어 Style이라고 부르며, 제약의 이름이다.
1.6 : 건축가 알렉산더의 철학을 참조하여 패턴에 대해서 정의. 패턴이란 특정 맥락에서 반복되는 문제를 해결하기 위한 구조적 해법. 패턴 언어는 여러 패턴들이 서로 연결되어 큰 설계 체계를 이루는 언어.
이를 통해, REST는 웹 환경에서 **자연스러운** 아키텍처 스타일이라고 주장함.
...
예를 들어,
	...
	시스템 수준(사용자 인증 시스템, 실제 서비스의 동작 방식) 
	> 컴포넌트 수준(인증 서비스, 토큰발급 서비스, 유저 정보를 관리하는 서비스 등의 모듈) 
	> 구현 수준(클래스나 메서드와 같은 실제 코드 단위)
...
예를 들어, Redis 캐시 서버를 왜 썼는지에 대한 근거가 설계서에 적혀있다고, 실제로 실행할 때 이 근거를 가지고 실행되는게 아니기 때문으로 이해할 수 있습니다.
...
여기서 스타일은, 우리가 흔히 아는 "개성"과는 사뭇 다릅니다.

로이 필딩은 아키텍처 제약의 집합을 패턴화한 것을 스타일이라고 주장합니다.

쉽게 말해, 특정 구조와 행동의 제약을 묶어서 이름을 붙인 것을 본 논문에서는 아키텍처의 스타일이라고 하겠다는 겁니다.

디자인 패턴이 코드 단위의 패턴을 나타낸다면, 아키텍처 스타일은 시스템 단위의 패턴을 뜻합니다.

바로 앞 1.4절에서, 뛰어난 품질은 제약에서 나온다고 언급된 바 있습니다. 이렇게 제약들이 일정 방식으로 묶이게 되면 이것이 하나의 스타일이 됩니다.

본 논문에서 소개하게 될 REST의 핵심 제약인 무상태성, 일관된 인터페이스, 캐시가능성, 계층화된 시스템, 클라이언트-서버 구조라는 제약들이 REST **Style**이라는 이름을 갖게 되는 것입니다.

그리고 여기서 로이 필딩은 모든 것을 단편적으로 보지 말라고 경고합니다.

하나의 아키텍처 스타일로 모든 문제를 해결할 수 없다고 말하는데, 쉽게 말해 REST나 MSA로 설계하면 모든 게 해결된다는 건 잘못된 시각이라는 겁니다.
...
을 고려해서 적합한 스타일을 선택 또는 조합해야 한다고 말합니다.

스타일은 **제약의 이름**으로, 감성적(개인적) 표현이 아니라는 것을 다시 한번 로이 필딩은 강조합니다.

그는 디자인의 영역에서도 이러한 사례를 들며 강조하는데, 전문 건축 영역에서조차 개인적 스타일이 개입될 여지가 없다는 예시를 듭니다.

그렇기 때문에, 아키텍처 스타일 = 요소들(컴포넌트, 커넥터, 데이터)의 관계를 제약하는 규칙의 집합이며, 이러한 규칙의 집합들에서 파생되는 공통된 속성에 이름을 붙이는 것을 스타일이라고 정의합니다.

즉, 우리가 현대에 마주하고 있는 소프트웨어들의 아키텍처는 이러한 스타일들이 섞인 하이브리드 스타일이라는 것입니다.

## 1.6 Patterns and Pattern Languages

이 절은 디자인 패턴과 아키텍처 스타일을 구분합니다.

디자인 패턴은 코드 수준이고, 아키텍처 스타일은 시스템 수준이라는 점에서부터 차이를 보입니다.

둘 다 **제약으로 문제 해결**이라는 점에서 같은 뿌리를 공유합니다.

패턴이란, **반복적으로** 나타나는 force와 context를 해결하는 구조라고 정의합니다.

	Context: 시스템이 운영되는 환경
	Force: 해결해야 할 요구사항, 성능, 안정성, 보안 등의 긴장관계
	Configuration: Force를 해결하기 위한 구조적 형태

이 세 가지가 반복적으로 나타나면 이것을 "패턴"이라고 부른다고 주장합니다.

그는 여기서 알렉산더라는 건축가의 철학을 인용하며, 스타일은 단순한 제약 모음이 아니라, 자연스럽게 협력 가능한 행동의 패턴임을 강조합니다.

* 1.7부터 1.9까지는 챕터 1을 요약한 부분이라서 생략하였습니다 *
