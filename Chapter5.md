...
클라이언트-서버 구조로 컴포넌트 구현이 단순화되고 의미론을 간소화하여, 성능 최적화와 확장성을 향상시킵니다.

REST는 메시지에 자신의 정보를 모두 담는 자기기술적 제약으로 중간 계층에서 요청을 이해하고 처리할 수 있습니다.

상호작용은 stateless하여 각 요청이 독립적이여서 전체 시스템을 알 필요가 없고, 표준 인터페이스와 미디어타입으로 의미가 전달되며, 응답은 캐시 가능성을 명시합니다.

또한 계층 구조의 제약으로 중간자를 어디에서나 추가할 수 있고, 추상화된 인터페이스가 변경되지 않기 때문에 다양한 방법으로 성능 향상에 기여할 수 있습니다.

컴포넌트들이 동적으로 연결되어 특정 요청에 따라 배치와 역할이 바뀌게 되는 Pipe-and-Filter스타일과 유사한 구조를 가지고 있습니다.

요청과 응답은 필터 체인을 따라 흐르며 필요한 곳에 컴포넌트를 추가할 수 있습니다.

정리하면, 이러한 장점들로 프로세스적 관점에서 구조가 유연하고 확장성이 높다고 주장합니다.

### 5.3.2 Connector View

컴포넌트 간 통신이 실제 어떤 방식으로 이루어지는지에 대해서 다룹니다.

REST 아키텍처는 Generic Resource Interface(일반화된 리소스 인터페이스)를 정의하는 제약조건을 핵심으로 합니다.

REST가 제약하는 건 오직 인터페이스 형태와 의미입니다.

즉, 특정 프로토콜에 한정된다는 것이 아닙니다.

웹에서는 주로 HTTP 프로토콜을 사용하나, REST는 FTP를 사용하는 서버에서도 동일한 인터페이스로 접근할 수 있어야된다고 강조합니다.

핵심은 프로토콜이 아닌, 모든 통신에서 단일 인터페이스로 모든 자원 접근이 가능하다는 것입니다.

### 5.3.3 Data View

데이터가 컴포넌트를 통과하면서 앱 상태가 어떻게 변하는지 보여줍니다.

상태는 앱이 **현재 어디서 무엇을 하는지**를 나타내는 정보입니다.

상태(State)는 

	- 현재 진행 중인 요청
	- 연결된 컴포넌트 구조(topology)
	- 각 요청의 데이터 흐름
	- 응답으로 전달되는 리소스 표현
	- User Agent가 그 표현을 처리

로 구성됩니다.

이를 바탕으로

	안정 상태: 서버에는 대기 중인 요청이 없고 클라이언트에는 모든 응답이 수신되어 랜더링이 가능한 상태에서 두 상태 사이의 지연으로 측정됩니다.

REST는 분산 시스템에 초점을 두므로, **하나의 앱 = 정보와 제어의 구조적 묶음**입니다.

필딩은 그 예로,

온라인 사전 단어 찾기, 가상 박물관 탐험, 강의노트보며 공부 등의 동작이 하나의 "애플리케이션 상태"라고 봅니다.

제어 의미는 작은 메시지로 전달되나 리소스 표현은 대용량 메시지로 전달되고, 대부분의 요청은 GET, 즉 리소스 표현을 가져오는 형태로 캐싱되어 저장된다는 것이 데이터 흐름에서의 특징입니다.

쉽게 말해, 앱의 상태는 클라이언트가 현재 어떤 화면(리소스 표현)에 머물러 있는지이고, 리소스 상태는 서버가 가진 현재 데이터 값입니다.

REST의 핵심 원칙인 상태를 서버가 아닌 클라이언트가 관리한다는 것은, 

서버가 각 요청을 독립적으로 처리하고 앱의 현재 상태를 클라이언트가 보유하여, 서버는 오직 현재 요청만 이해하면 된다는 점입니다.

덕분에 서버는 확장성에서 큰 이점을 얻습니다.

가장 빠른 네트워크 요청은 네트워크를 사용하지 않는 요청으로,

캐싱된 데이터를 응답으로 재사용할 수 있으면 전체 지연 시간이 크게 줄어들게 됩니다.

오버헤드가 발생할 수는 있으나, 평균 지연 시간은 대폭 감소합니다.

REST의 상태 전이는 User Agent가 HATEOAS를 따라 다음 상태로 이동할 링크를 선택하며 넘어가는 것을 말합니다.

User Agent는 앱의 상태를 저장하고 제어합니다. 여러 서버들의 표현을 결합할 수도 있고, 상태를 직접 조작할수도 있습니다.

데이터의 관점에서 REST 애플리케이션의 본질은, **하나의 상태에서 다른 상태로 이동**하는 엔진이라는 것입니다.

**각 상태는 리소스의 표현으로 나타내지고, 상태 전이는 링크를 통한 요청**으로 이루어지게 됩니다.

다시 한번 REST의 약어를 살펴보면,

REST = Representational State Transfer, 표현을 통한 상태 전이라는 뜻으로 -> 서버는 리소스 표현을 클라이언트에게 전달하고 클라이언트는 그걸 해석하여 앱의 상태를 바꾼다는 것입니다.

더욱 간단하게 정리하면, **특정 형식으로 현재 데이터를 전달하여 상태를 바꾼다**라고 정리할 수 있습니다.

	- Representation: 서버에서 클라이언트로 전송되는 데이터(ex. JSON, HTML 등)
	- Resource State: DB에 저장된 데이터
	- Application State: 현재 어떤 링크를 클릭했는가, 어떤 리소스를 보고 있는가
	- State Transition: 하이퍼링크를 통해 새로운 리소스로 이동하는 클라이언트의 행위
	
그러나 현대의 REST는, 이러한 상태 전이의 개념이 많이 바뀌었습니다.

이유는, 세 가지가 있습니다.

	1. 하이퍼미디어 브라우징 구조가 클라이언트에서는 필요하지 않음. React의 등장으로 State는 전부 프론트엔드가 관리하게 되었습니다. 그렇기에 굳이 link를 줄 필요가 사라졌습니다.

	2. API 클라이언트는 결국 자동화된 시스템. 링크를 따라 사람이 직접 클릭하여 이동하는 것이 아닌, 특정 기능을 호출하는 방식으로 동작합니다. 즉, 상태 전이가 아닌 **비즈니스 작업 요청**이 중심입니다.

	3. 복잡한 하이퍼미디어 구조가 개발 효율이 떨어짐. 대규모 API시대가 되면서 서버는 리소스 표현에만 집중하고, 다음에 무엇을 할지는 클라이언트가 관리하게 되었습니다.

상태 전이는 그래서 현재 다른 리소스로의 이동이 아닌, **클라이언트의 상태 변화**로 해석되어 집니다.

즉, "페이지 이동"에서 **UI 갱신, 데이터 변경**에 초점이 맞춰지게 되었습니다.
